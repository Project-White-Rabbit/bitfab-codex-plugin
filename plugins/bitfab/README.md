...
Codex has no live-from-worktree mode — each rebuild must be vendored and copied into `~/.codex/plugins/cache/bitfab-dev/bitfab/local/`. Run the `/bitfab-codex-dev-build` skill to automate that. The `/bitfab-codex-toggle` skill switches between dev and prod variants (flag is user-global — Codex has no `--scope local`).
