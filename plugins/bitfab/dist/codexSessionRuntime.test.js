...
import { readCodexSessionRuntime, recordCodexSessionRuntime, resolveCodexSessionRuntime, runtimeServerPath, sessionRuntimePath, } from "./codexSessionRuntime.js";
let oldCodexHome;
let oldThreadId;
let oldAgentSessionId;
let oldSupersetWorkspacePath;
let oldSessionLogPath;
let tmp;
let repo;
beforeEach(() => {
    ...
    tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "codex-runtime-")));
    ...
    fs.writeFileSync(path.join(repo, "bitfab-codex-plugin", "dist", "mcp", "server.js"), "console.log('server')\n");
});
afterEach(() => {
...
describe("codex session runtime", () => {
    it("records a session-scoped worktree runtime from hook input", () => {
        ...
        expect(JSON.parse(fs.readFileSync(sessionRuntimePath("session-a"), "utf8"))).toMatchObject(runtime ?? {});
    });
    ...
    it("returns the worktree MCP server path when it exists", () => {
        ...
        expect(runtime ? runtimeServerPath(runtime) : null).toBe(path.join(repo, "bitfab-codex-plugin", "dist", "mcp", "server.js"));
    });
});
