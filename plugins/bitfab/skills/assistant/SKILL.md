...
20. **Run only when mode is `fix`.**
   ...
   Run node "${BITFAB_PLUGIN_DIR}/dist/commands/pageLink.js" /experiments?testRunIds=<testRunId>&autoOpenFirst=1 (quote the entire path) and relay the returned URL so the user can inspect the before/after. Return to the fix decision in chat.
