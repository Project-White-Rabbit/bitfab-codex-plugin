...
13. **🚨 A trace plan is always delivered the same way: build the tree in memory, post it with `mcp__Bitfab__create_trace_plan`, render it inline as ASCII (using the "Trace Plan Format" reference section below), then ask the user whether the user wants to review it in the browser (Studio) or just continue.** When the user says "create a trace plan", "give me the trace plan", "show me the trace plan", or "bitfab create a plan", that is a request to run this createTracePlan → render-ASCII → ask path, never a request to skip past the plan or to hand-type a plan you made up outside this flow. The inline ASCII IS the plan the user reviews in chat; Studio is an optional richer surface for reviewing and adjusting the captured set, offered every time but never forced. Build the tree in memory first, then call `mcp__Bitfab__create_trace_plan`, then render.
   ...
   **Polling (mandatory, see the Blocking-process rule at the top of this skill):** keep polling the live exec session for its terminal line; the user's Close/Update arrives as stdout on the already-running process, not as a chat message.
     ...
     - **Nothing yet, and they typed a change instead:** close the open plan with `node "${BITFAB_PLUGIN_DIR}/dist/commands/openStudioTo.js" "/studio"`, stop the background process, then rebuild the tree and start this step over from `mcp__Bitfab__create_trace_plan`. The superseded plan needs no cleanup; it expires on its own.
...
5. **Post the modified plan, render it inline as ASCII, then ask the user whether to review it in the browser or just continue**, same delivery pattern as Instrument's build-trace-plan step. The inline ASCII is what the user reviews in chat; Studio is the optional richer surface where they can adjust the captured set (selecting/deselecting any of the surrounding `pure` context nodes added in step 4). Continuing in chat, and every way of leaving the plan page in Studio (**Close**, **Update**, closing the window), applies the diff. None of them is an abort or a request for another round of plan edits.
   ...
   **If `mcp__Bitfab__create_trace_plan` itself errors** (offline or MCP unreachable, so there is no plan id or browser option): render the inline before/after ASCII from your in-memory tree, derive the mock decisions yourself, and **STOP**: ask the user using the options above before writing edits.
...
After building the plan and posting it with `mcp__Bitfab__create_trace_plan`, render it inline as ASCII (rules above), then ask the user:
