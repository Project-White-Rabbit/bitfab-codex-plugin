---
name: update
description: "Update the Bitfab Codex plugin and/or the Bitfab SDK installed in the current project. Invoke with $bitfab:update [plugin|sdk|all]."
---
...
**Skip this step if mode is `sdk`.** If the plugin was updated, remind the user to restart Codex to apply the update. If the mode was `plugin`, stop here — do not run steps 3-6.
...
For each entry with `updateAvailable: true` (after step 4 + 5 reconciliation), ask the user — **one decision per question**:
