...
import type { HookInput } from "bitfab-plugin-lib"

interface CodexSessionRuntime {
  sessionId: string
  worktree: string
  bitfabRuntime: string
  bitfabDevRuntime: string
  recordedAt: string
}
...
function codexSessionId(input?: Pick<HookInput, "session_id"> | null) {
...
export function sessionRuntimePath(sessionId: string): string {
...
function cwdFromSessionLog(): string | null {
...
function runtimeForWorktree(worktree: string): CodexSessionRuntime {
...
export function recordCodexSessionRuntime(
...
export function readCodexSessionRuntime(
  sessionId = codexSessionId() ?? undefined,
): CodexSessionRuntime | null {
  if (!sessionId) {
    return null
  }
  try {
    const parsed = JSON.parse(
      fs.readFileSync(sessionRuntimePath(sessionId), "utf-8"),
    ) as CodexSessionRuntime
    if (
